...
describe('select', function() {
    ...
    it('real', function() {
        expect("SELECT raw_data FROM sso_service_lines WHERE (created_at > NOW() - INTERVAL '1 minute' OR updated_at > NOW() - INTERVAL '1 minute') AND portal_id = :portal_id`;")
            .to.be.equal(sql
                .select('raw_data', 'sso_service_lines')
...
describe('delete', function() {
    it('simple', function() {
        expect('DELETE FROM test;').to.be.equal(sql.delete('test').sql());
    });

    it('where', function() {
        expect('DELETE FROM test WHERE id=:id;').to.be.equal(sql.delete('test').where({id: ':id'}).sql());
        expect('DELETE FROM test WHERE id=:id;').to.be.equal(sql.delete('test').where('id=:id').sql());
    });
});
...
describe('update', function() {
    it('simple', function() {
        expect('UPDATE test SET id=:id;').to.be.equal(sql.update('test', {id: ':id'}).sql());
    });

    it('where', function() {
        expect('UPDATE test SET id=:id WHERE name=:name;').to.be.equal(sql.update('test', {id: ':id'}).where('name=:name').sql());
        expect('UPDATE test SET id=:id WHERE name=:name;').to.be.equal(sql.update('test', {id: ':id'}).where({name: ':name'}).sql());
    });
});
